...
static void eval_layer_tc43(const uint64_t *in, uint64_t *out, int seg_len)
{
...
static const InterpParamTC43 INTERP_PARAMS_TC43[] = {
...
static int interp_param_idx_tc43(int stride)
{
    switch (stride)
    {
        case 4: return 0;
...
static void interp_layer_tc43(const uint64_t *w, uint64_t *c, int stride)
{
...
static void product4_tc43(const uint64_t *a, const uint64_t *b, uint64_t *c)
{
...
static void core16_tc43(const uint64_t *a16, const uint64_t *b16, uint64_t *c16)
{
...
static inline uint64_t tc4_eval_point(uint64_t r0, uint64_t r1, uint64_t r2, uint64_t r3, int point_idx)
{
    switch (point_idx)
    {
        case 0: return r3;
        case 1: return (r3 << 3) + (r2 << 2) + (r1 << 1) + r0;
        case 2: return r0 + r1 + r2 + r3;
        case 3: return (r0 + r2) - (r1 + r3);
        case 4: return (((r0 << 2) + r2) << 1) + ((r1 << 2) + r3);
        case 5: return (((r0 << 2) + r2) << 1) - ((r1 << 2) + r3);
        case 6: return r0;
        default: return 0;
    }
}

static inline uint64_t load_coeff_4d(const uint64_t *in, int i, int j, int k, int l)
{
    return in[64 * l + 16 * k + 4 * j + i];
}

static void build_eval_vector16_striding(
    const uint64_t *in,
    int pt0,
    int pt1,
    int pt2,
