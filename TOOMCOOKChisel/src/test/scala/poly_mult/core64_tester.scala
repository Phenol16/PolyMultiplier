...
class core64Test extends AnyFlatSpec with ChiselScalatestTester {
  private val N = 64
  private val QMask: BigInt = (BigInt(1) << Core64TestParams.AWidth) - 1
  ...
  private def runCase(
    ...
    var cycle = 0
    while (!dut.io.valid_out.peek().litToBoolean && cycle < 200) {
      dut.clock.step(1)
      cycle += 1
    }
  ...
  .withAnnotations(Seq(VerilatorBackendAnnotation)) { dut =>
    ...
    dut.io.valid_in.poke(false.B)
      ...
      for (trial <- 0 until 10) {
        val a = Seq.fill(N)(BigInt(Core64TestParams.AWidth, rand))
