...
class core4Test extends AnyFlatSpec with ChiselScalatestTester {
  private val N = 4
  private val QMask: BigInt = (BigInt(1) << Core4TestParams.AWidth) - 1
  private val BMask: BigInt = (BigInt(1) << Core4TestParams.BWidth) - 1
  ...
  private def runCase(
    ...
    var cycle = 0
    while (!dut.io.valid_out.peek().litToBoolean && cycle < 50) {
      dut.clock.step(1)
      cycle += 1
    }
  ...
  it should "match the negacyclic schoolbook reference" in {
    test(Core4TestParams.dut) { dut =>
      ...
      runCase(
        dut,
        "single_a0_b0",
        Seq.tabulate(N)(i => if (i == 0) BigInt(1) else BigInt(0)),
        Seq.tabulate(N)(i => if (i == 0) BigInt(1) else BigInt(0))
      )
      ...
      runCase(
        dut,
        "single_wrap",
        Seq.tabulate(N)(i => if (i == N - 1) BigInt(1) else BigInt(0)),
        Seq.tabulate(N)(i => if (i == N - 1) BigInt(1) else BigInt(0))
      )
      ...
      for (trial <- 0 until 10) {
        val a = Seq.fill(N)(BigInt(Core4TestParams.AWidth, rand))
        val b = Seq.fill(N)(BigInt(Core4TestParams.BWidth, rand))
