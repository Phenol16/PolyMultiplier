...
class evaluation extends Module {
  ...
  for (j <- 0 until 4) {
    ...
    io.A_eval(j * 7 + 3) := a_even - a_odd
    io.A_eval(j * 7 + 4) := Cat(a_scaled_even, 0.U(1.W)) + a_scaled_odd
    io.A_eval(j * 7 + 5) := Cat(a_scaled_even, 0.U(1.W)) - a_scaled_odd
    io.A_eval(j * 7 + 6) := ar0
    ...
    io.B_eval(j * 7 + 3) := b_even - b_odd
    io.B_eval(j * 7 + 4) := Cat(b_scaled_even, 0.U(1.W)) + b_scaled_odd
    io.B_eval(j * 7 + 5) := Cat(b_scaled_even, 0.U(1.W)) - b_scaled_odd
    io.B_eval(j * 7 + 6) := br0
  }
