package core
import chisel3._
import chisel3.util._
class Interpolation(
    stride: Int,
    inWidth: Int,
    outWidth: Int,
) extends Module {
  val io = IO(new Bundle {
    val valid_in = Input(Bool())
    val w = Input(Vec(7 * stride, UInt(inWidth.W)))
    val valid_out = Output(Bool())
    val c = Output(Vec(4 * stride, UInt(outWidth.W)))
  })

  val interp = Module(new InterpolationComb(stride, inWidth, outWidth))
  interp.io.w := io.w

  val validReg = RegNext(io.valid_in, false.B)
  val cReg = RegInit(VecInit(Seq.fill(4 * stride)(0.U(outWidth.W))))

  when (io.valid_in) {
    cReg := interp.io.c
  }

  io.valid_out := validReg
  io.c := cReg
}

class InterpolationComb(
    stride: Int,
    inWidth: Int,
    outWidth: Int,
) extends Module {
  val io = IO(new Bundle {
    val w = Input(Vec(7 * stride, UInt(inWidth.W)))
    val c = Output(Vec(4 * stride, UInt(outWidth.W)))
  })

  private val inv3 = MagicNumber.inv3(outWidth)
  private val inv9 = MagicNumber.inv9(inWidth-2)
  private val inv15 = MagicNumber.inv15(inWidth-2)

  val cRaw = Wire(Vec(4 * stride, UInt(outWidth.W)))
  val prevR0 = Wire(Vec(stride + 1, UInt(inWidth.W)))
  val prevR1 = Wire(Vec(stride + 1, UInt(inWidth.W)))
  val prevR2 = Wire(Vec(stride + 1, UInt(inWidth.W)))

  prevR0(0) := 0.U
  prevR1(0) := 0.U
  prevR2(0) := 0.U

  for (i <- 0 until stride) {
    val p0 = io.w(i)
    val p1 = io.w(stride + i)
    val p2 = io.w(2 * stride + i)
    val p3 = io.w(3 * stride + i)
    val p4 = io.w(4 * stride + i)
    val p5 = io.w(5 * stride + i)
    val p6 = io.w(6 * stride + i)

    val r1a = p1 + p4
    val r5a = p5 - p4
    val r3a = (p3 - p2) >> 1
    val r4a = p4 - p0
    val r4b = (r4a << 1) + r5a - (p6 << 7)
    val r2a = p2 + r3a
    val r1b = r1a - (r2a << 6) - r2a
    val r2b = r2a - p6 - p0
    val r1c = r1b + r2b + (r2b << 2) + (r2b << 3) + (r2b << 5)

    val r4d = ParaMath.mask(
        ((r4b - (r2b << 3)) >> 3) * inv3.U,
      outWidth
    )
    val r5c = ParaMath.mask(
      ParaMath.mask((r5a + r1c) >> 1, inWidth - 2) * inv15.U,
      inWidth-2
    )
    val r1e = ParaMath.mask(
      ((r1c + (r3a << 4)) >> 1) * inv9.U,
      inWidth-2
    )

    val r2c = ParaMath.mask(r2b - r4d, outWidth)
    val r3b = ParaMath.mask(0.U - r1e - r3a, outWidth)
    val r5d = ParaMath.mask((r1e - r5c) >> 1, outWidth)
    val r1f = ParaMath.mask(r1e - r5d, outWidth)

    cRaw(4 * i + 0) := p6 + prevR2(i)
    cRaw(4 * i + 1) := r5d + prevR1(i)
    cRaw(4 * i + 2) := r4d + prevR0(i)
    cRaw(4 * i + 3) := r3b

    prevR0(i + 1) := ParaMath.mask(p0, outWidth)
    prevR1(i + 1) := r1f
    prevR2(i + 1) := r2c
  }

  for (i <- 0 until 4 * stride) {
    io.c(i) := cRaw(i)
  }
  io.c(0) := cRaw(0) - prevR2(stride)
  io.c(1) := cRaw(1) - prevR1(stride)
  io.c(2) := cRaw(2) - prevR0(stride)
}