...
analyze -format sverilog -vcs "+define+TSMC28HPCP" ${RTL_SOURCE_FILES}
...
#read_ddc ${RESULTS_DIR}/${DCRM_ELABORATED_DESIGN_DDC_OUTPUT_FILE}
...
#group_path -name clk_main_gp -from "clock_main" -to [all_outputs]
#group_path -name clk_cam_gp  -from "clock_cam" -to [all_outputs]
...
#######qiang : clock_gating_style ########
#set_clock_gating_style -positive_edge_logic {integrated:tcbn28hpcplusbwp40p140lvtssg0p81vm40c_ccs/CKLNQD2BWP40P140LVT} -negative_edge_logic {integrated:tcbn28hpcplusbwp40p140lvtssg0p81vm40c_ccs/CKLHQD2BWP40P140LVT} -control_point before
    ...
    #read_saif -auto_map_names -input ${DESIGN_NAME}.saif -instance Tb.Dut -verbose

    if {[shell_is_in_topographical_mode]} {
      # For multi-Vth design, replace the following to set the threshold voltage groups in the libraries.

       set_attribute [get_lib *hvt*] default_threshold_voltage_group HVT -type string
       set_attribute [get_lib *lvt*] default_threshold_voltage_group LVT -type string
    }
...
set_fix_multiple_port_nets -all -no_rewire
...
########### qiang : taget_library subset for synchro
#tcbn28hpcplusbwp40p140hvtssg0p81vm40c_ccs
#set_target_library_subset -object_list $synchronizer_reg -use [concat [get_object_name [get_lib_cells */SDFSYNC*]] [get_object_name  [get_lib_cells */INVD*]] [get_object_name  [get_lib_cells */TIE*]] [get_object_name  [get_lib_cells */AN2*]] [get_object_name  [get_lib_cells */OR2*]] [get_object_name  [get_lib_cells */NR2*]]]

#[concat [get_object_name [get_lib_cells */SDFSYNC*]] [get_object_name  [get_lib_cells */INVD*]] [get_object_name  [get_lib_cells */TIE*]]]

#set compile_seqmap_propagate_constants false

#insert_clock_gating

### Starting from 2013.12 release, constant propagation is enabled even when boundary optimization is disabled.
...
report_constraint -all_violators -verbose > ${REPORTS_DIR}/${DESIGN_NAME}.vio.rpt
report_timing -delay_type max -max 3 > ${REPORTS_DIR}/${DESIGN_NAME}.max_timing.rpt
report_timing -delay_type min -max 3 > ${REPORTS_DIR}/${DESIGN_NAME}.min_timing.rpt
}
...
report_power -hierarchy > ${REPORTS_DIR}/${DESIGN_NAME}.hier.power.result
report_power -verbose > ${REPORTS_DIR}/${DESIGN_NAME}.power.result
report_power -hierarchy -level 1 > ${REPORTS_DIR}/${DESIGN_NAME}.hier.power.level1.result
report_power -hierarchy -level 2 > ${REPORTS_DIR}/${DESIGN_NAME}.hier.power.level2.result
